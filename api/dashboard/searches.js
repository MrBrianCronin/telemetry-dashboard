...
export default async function handler(req, res) {
  ...
  try {
    const searches = await sql`
      ...
      FROM events
      WHERE event_type = 'interest_search' AND created_at > NOW() - make_interval(days => ${daysInt})
        AND event_data->>'query' IS NOT NULL
      GROUP BY event_data->>'query' ORDER BY count DESC LIMIT 20
    `;
    ...
    const matchedCategories = await sql`
      ...
      GROUP BY match_label ORDER BY count DESC LIMIT 15
    `;
